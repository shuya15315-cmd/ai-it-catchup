<#
  ...
  ------------------------------------------------------------------
  このPC（Claude Code 入り）で「セットアップ.bat」をダブルクリック
  （または本ファイルをPowerShellで実行）してください。
    ・毎週月曜 8:00 の自動更新タスクを登録
    ・デスクトップに閲覧用ショートカットを作成
  ※ このスクリプトは自分の置き場所を自動認識するので、どこに置いても・移動してもOKです。
     （フォルダを移動したら、もう一度これを実行すればタスクとショートカットが直ります）
#>
try { chcp 65001 > $null; [Console]::OutputEncoding = [System.Text.Encoding]::UTF8 } catch {}

$proj = Split-Path -Parent $MyInvocation.MyCommand.Path
$taskName = "AI-IT情報キャッチアップ_自動更新"
Write-Host "==== AI・IT 情報キャッチアップ セットアップ ===="
...
try {
  Register-ScheduledTask -TaskName $taskName -Action $action -Trigger $trigger -Settings $settings -Principal $principal `
    -Description "毎週月曜8:00に公開情報からAI・ITの最新情報を集めてdata.jsを更新（公開情報を読むだけ）。" -Force -ErrorAction Stop | Out-Null
  Write-Host "[2/3] 自動更新タスクを登録: 毎週月曜 8:00  → OK"
} catch {
  Write-Host ("[2/3] 【注意】タスク登録に失敗: " + $_.Exception.Message) -ForegroundColor Yellow
  Write-Host "      （管理者権限が必要な場合があります。タスクスケジューラで手動登録も可能です）"
}

# 3) デスクトップショートカット
try {
  $ws = New-Object -ComObject WScript.Shell
  $lnk = $ws.CreateShortcut((Join-Path ([Environment]::GetFolderPath("Desktop")) "AI・IT情報キャッチアップ.lnk"))
  $lnk.TargetPath = Join-Path $proj "index.html"
  $lnk.WorkingDirectory = $proj
  $lnk.Description = "AI・IT 情報キャッチアップ"
  $lnk.Save()
