...
$prompt = "Read the file 'update-prompt.md' in this directory and follow its instructions exactly to refresh 'data.js' with the latest AI and IT information for a non-engineer audience in Japanese. Use only web search and public sources. Do not send any private data."
...
if ($code -eq 0 -and (Get-Command git -ErrorAction SilentlyContinue)) {
  ...
  if ($LASTEXITCODE -eq 0) {
    ...
    if ($LASTEXITCODE -ne 0) {   # 変更あり
      ...
      if ($LASTEXITCODE -eq 0) { Write-Log "GitHub へ push 完了" }
